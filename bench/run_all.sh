...
# `burst_window_ms=100` pair gates the burst detector so
# that typical rm -rf workloads (10/100/500 files) stay in
# burst mode. After issue #568 stage 6 the deleter has no
# flush-coalescing window — every enqueue fires a single
# DELETE — so the bench measures pure N-concurrent DELETE
# wall time on the batched mount, not batch XML overhead.
...
# Historical knobs (MNTRS_BATCH_FLUSH_DELAY_MS,
# MNTRS_BATCH_THRESHOLD, MNTRS_BATCH_SIZE, MNTRS_BATCH_PROFILE)
# were removed in issue #568 stage 6 / #570 — the batched
# deleter is now rclone-style N concurrent single-DELETE.
# The default policy at runtime is unchanged.
echo "  $(date -Iseconds): starting mntrs-concurrent mount (MNTRS_UNLINK_BATCH=1, caller-side burst tuning)..."
export MNTRS_BATCH_DAEMON_LOG="${MNTRS_BATCH_DAEMON_LOG:-/tmp/mntrs-daemon-batch.log}"
# Issue #572 follow-up: the tracing target renamed from
# `mntrs::batched_delete` to `mntrs::concurrent_delete`.
