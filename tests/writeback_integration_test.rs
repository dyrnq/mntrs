...
use mntrs::{writeback, FileType, InodeEntry, Inodes};
...
impl Harness {
    ...
    fn enqueue(&self, ino: u64, remote: &str, cache_path: PathBuf) {
        self.sender.send((ino, remote.to_string(), cache_path, 0)).unwrap();
    }
...
fn happy_path_uploads_and_removes_dirty_sidecar() {
    ...
    assert!(h.wait_drain(&dirty, 5_000), "dirty sidecar removed within 5s");
...
fn multiple_files_upload_in_order_or_out_of_order_but_all_complete() {
    ...
    for d in &dirtys {
        assert!(h.wait_drain(d, 10_000), "all .dirty sidecars removed within 10s");
    }
...
fn writeback_pending_set_cleared_after_upload() {
    ...
    h.writeback_pending.insert("/remote/pending.bin".to_string());
